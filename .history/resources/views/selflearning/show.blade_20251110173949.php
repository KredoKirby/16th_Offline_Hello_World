...
<div class="container">
    ...
    <div class="accordion course-accordion" id="courseAccordion">
        @foreach($course->topics as $index => $top)
            <div class="accordion-item">
                <h2 class="accordion-header" id="heading{{ $index }}">
                    <button class="accordion-button {{ $index > 0 ? 'collapsed' : '' }}" type="button"
                        data-bs-toggle="collapse" data-bs-target="#collapse{{ $index }}">
                        <span class="fw-bold">{{ $top->title }}</span>
                      @php
                        // 各topの全lessonのduration合計（秒）
                        $totalSeconds = $top->lessons->sum('duration');
                        $minutes = floor($totalSeconds / 60);
                        $seconds = $totalSeconds % 60;
                        $formattedDuration = sprintf('%02d:%02d', $minutes, $seconds);
                    @endphp

                    <span class="ms-auto top-meta text-muted small">
                        {{ $top->lessons->count() }} lectures ・
                        {{ $top->lessons->sum('pages') }} pages ・
                        {{ $formattedDuration }}
                    </span>
